...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/prescription/generate", authenticateJWT, authorizeRoles(UserRole.enum.Doctor), async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      // PDF Generation is temporarily disabled due to missing pdfkit package.
      // All PDF generation logic below is currently bypassed.
      return res.status(501).send("PDF generation is temporarily disabled due to missing pdfkit package.");
